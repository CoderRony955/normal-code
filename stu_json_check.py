...
def main():
    ...
    students = load_student_details('Students.json') #<----Take input from the user for student name and we use function
    name = input("Enter your name: ")
#check if the name was present in Student_detials or not using if ele condtion
    if(name in students):
        print(f"Hello {name}, here is your details now you can check you total percentage or total marks :>")
        print()
        print(students[name])#<--------- show student details which entered by the user 
    else:
...
#then take inputs from the user for each subjects marks
    ...
    """now we use if condtion for "pass" or "fail" but we can make it complicate like if student score less than 33 in anyone subject only then the student will failed like student must be need to score 33 i every subject to pass
    """
